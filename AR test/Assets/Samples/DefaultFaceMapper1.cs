...
using UnityEngine;
    ...
    /// <summary>
    /// The default <see cref="MapperFace"/> implementation.
    /// </summary>
    /// <remarks>
    ...
    /// all skinned meshes relative to the <see cref="Controller"/> components are consistent between the rigs.
    /// </remarks>
    [CreateAssetMenu(fileName = "NewFaceMapper", menuName = "mappppppppperrrrrr/skrrrr")]
    public class DefaultFaceMapper1 : MapperFace
    {
...
#pragma warning disable CS0414
        ...
        EvulatorPreset2 m_DefaultEvaluator = null;
...
#pragma warning restore CS0414
        ...
        struct BindingData
        {
            ...
            public BindingData(int shapeIndex)
            {
        ...
        {
            public readonly FaceShapeBlend Location;
            public readonly BindingData[] Bindings;

            public MappingData(FaceShapeBlend location, BindingData[] bindings)
            {
                Location = location;
                Bindings = bindings;
            }
        ...
        readonly struct RendererData
        {
            ...
            public RendererData(Controller actor, RendererMapping rendererMapping)
            {
        ...
        class Cache : FaceCacheMapper
        {
            ...
            public Cache(Controller actor, IEnumerable<RendererMapping> maps)
            {
            ...
            public bool TryGetBone(Controller actor, string path, out Transform bone)
            {
        ...
        /// <inheritdoc />
        public override FaceCacheMapper CreateCache(Controller actor)
        {
        ...
        )
        {
            ...
            if (!string.IsNullOrEmpty(m_LeftEye) && c.TryGetBone(actor, m_LeftEye, out var leftEye))
            {
                switch (m_EyeMovementDriver)
                {
                    case EyeMovementDriverType.BlendShapes:
                        {
                            var leftEyeH = pose.GetValue(FaceShapeBlend.EyeLookInLeft) - pose.GetValue(FaceShapeBlend.EyeLookOutLeft);
                            var leftEyeV = pose.GetValue(FaceShapeBlend.EyeLookUpLeft) - pose.GetValue(FaceShapeBlend.EyeLookDownLeft);
            ...
            if (!string.IsNullOrEmpty(m_RightEye) && c.TryGetBone(actor, m_RightEye, out var rightEye))
            {
                switch (m_EyeMovementDriver)
                {
                    case EyeMovementDriverType.BlendShapes:
                        {
                            var rightEyeH = pose.GetValue(FaceShapeBlend.EyeLookInRight) - pose.GetValue(FaceShapeBlend.EyeLookOutRight);
                            var rightEyeV = pose.GetValue(FaceShapeBlend.EyeLookUpRight) - pose.GetValue(FaceShapeBlend.EyeLookDownRight);
    ...
    public override void ApplyBlendShapesToRig(Controller actor, FaceCacheMapper cache, ref FaceShapeBlendPose pose, bool continuous)
    {
        throw new NotImplementedException();
    }

    public override void ApplyHeadPositionToRig(Controller actor, FaceCacheMapper cache, ref Vector3 headPosition, bool continuous)
    {
        throw new NotImplementedException();
    }

    public override void ApplyHeadRotationToRig(Controller actor, FaceCacheMapper cache, ref Quaternion headOrientation, bool continuous)
    {
        throw new NotImplementedException();
    }

    public override void ApplyEyeRotationToRig(Controller actor, FaceCacheMapper cache, ref FaceShapeBlendPose pose, ref Quaternion leftEyeRotation, ref Quaternion rightEyeRotation, bool continuous)
    {
        throw new NotImplementedException();
    }
}
