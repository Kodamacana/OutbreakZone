...
public enum FaceShapeBlend : int
{
...
public partial struct FaceShapeBlendPose
{
    /// <summary>
    /// The face blend shapes, excluding shapes with a negative value.
    /// </summary>
    public static FaceShapeBlend[] Shapes { get; } = Enum.GetValues(typeof(FaceShapeBlend))
        .Cast<FaceShapeBlend>()
        .Where(s => (int)s >= 0)
        .ToArray();
    ...
    public float GetValue(FaceShapeBlend location) => GetValue((int)location);
    ...
    public void SetValue(FaceShapeBlend location, float value) => SetValue((int)location, value);
