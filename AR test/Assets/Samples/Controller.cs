...
public class Controller : MonoBehaviour
{
    ...
    FaceFlagsChannel m_EnabledChannels = FaceFlagsChannel.All;

    [SerializeField, Tooltip("The blend shapes weights that define the face expression.")]
    FaceShapeBlendPose m_BlendShapes;
    ...
    FaceCacheMapper m_Cache;
    FaceFlagsChannel m_LastChannels;
    ...
    /// </summary>
    public MapperFace Mapper => m_Mapper;

    internal FaceShapeBlendPose BlendShapes
    {
        get => m_BlendShapes;
    ...
    public void SetMapper(MapperFace mapper)
    {
        m_Mapper = mapper;
        ClearCache();
    }
    ...
    public void UpdateRig(bool continuous)
    {
        if (m_Mapper != null)
        {
            ...
            {
                channels &= ~FaceFlagsChannel.BlendShapes;
            }
            if (!m_HeadPositionEnabled)
            {
                channels &= ~FaceFlagsChannel.HeadPosition;
            }
            if (!m_HeadOrientationEnabled)
            {
                channels &= ~FaceFlagsChannel.HeadRotation;
            }
            if (!m_EyeOrientationEnabled)
            {
                channels &= ~FaceFlagsChannel.Eyes;
            }

            // apply the active face channels
            if (channels.HasFlag(FaceFlagsChannel.BlendShapes))
            {
                m_Mapper.ApplyBlendShapesToRig(
                    this,
                    m_Cache,
                    ref m_BlendShapes,
                    continuous && m_LastChannels.HasFlag(FaceFlagsChannel.BlendShapes)
                );
            }
            if (channels.HasFlag(FaceFlagsChannel.HeadPosition))
            {
                m_Mapper.ApplyHeadPositionToRig(
                    this,
                    m_Cache,
                    ref m_HeadPosition,
                    continuous && m_LastChannels.HasFlag(FaceFlagsChannel.HeadPosition)
                );
            }
            if (channels.HasFlag(FaceFlagsChannel.HeadRotation))
            {
                ...
                m_Mapper.ApplyHeadRotationToRig(
                    ...
                    m_Cache,
                    ref headRotation,
                    continuous && m_LastChannels.HasFlag(FaceFlagsChannel.HeadRotation)
                );
            }
            if (channels.HasFlag(FaceFlagsChannel.Eyes))
            {
                ...
                m_Mapper.ApplyEyeRotationToRig(
                    ...
                    continuous && m_LastChannels.HasFlag(FaceFlagsChannel.Eyes)
                );
            }
    ...
    /// <inheritdoc/>
    public void Register(ViewerProperty previewer)
    {
        if (m_Mapper != null)
        {
        ...
        foreach (var shape in FaceShapeBlendPose.Shapes)
        {
